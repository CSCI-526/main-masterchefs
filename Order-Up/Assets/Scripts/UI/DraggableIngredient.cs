...
public class DraggableIngredient : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (enableDebugLogs)
            Debug.Log($"[{name}] DraggableIngredient initialized.");
    }
    ...
    void StopDragging()
    {
        ...
        if (dropZone != null)
        {
            ...
            else if (dropZone is Cookwares cookware)
            {
                ...
                // ✅ Make cookware dirty on drop
                var maintenance = cookware.GetComponent<CookwareMaintenance>();
                if (maintenance != null) maintenance.AddDirt(1);
            }
        }
        else
        {
    ...
    IDropZone GetDropZoneBelow()
    {
        var colliders = Physics2D.OverlapPointAll(transform.position, dropZoneLayerMask);
    ...
    void ReturnToOriginalPosition() => StartCoroutine(ReturnToPositionCoroutine());
    System.Collections.IEnumerator ReturnToPositionCoroutine()
    {
        Vector3 startPos = transform.position;
    ...
    public void SetNewOriginalPosition() => originalPosition = transform.position;
    public Vector3 GetOriginalPosition() => originalPosition;
    public void EnableDragging() => enabled = true;
}
